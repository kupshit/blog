...
function sl_enqueue_scripts() {
	...
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		...
		'like' => __( 'Like', 'myshare' ),
		'unlike' => __( 'Unlike', 'myshare' )
	) ); 
}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'myshare' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'myshare' );
		$icon = $icon_empty;
	}
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'myshare' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'myshare' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
