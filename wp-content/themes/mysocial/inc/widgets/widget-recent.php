 ...
 * @since      1.0.0
 */
class mysocial_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-mysocial-recent widget_posts_thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'mysocial' )
		);

		// Create the widget.
		parent::__construct(
			'mysocial-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'mysocial' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'mysocial_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'mysocial_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'mysocial_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'mysocial' ),
			'limit'     => 5,
			'show_date' => true
		);
